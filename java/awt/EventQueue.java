...
public class EventQueue {
	...
	if (queues[priority].head == null) {
		...
		for (;;) {
		    if ((theEvent instanceof PaintEvent) &&
                        (q.event instanceof PaintEvent) &&
			(q.event.getSource() == theEvent.getSource())) {
		        AWTEvent coalescedEvent = coalescePaintEvents(
                                     (PaintEvent)q.event, (PaintEvent)theEvent);
    ...
    private PaintEvent coalescePaintEvents(PaintEvent oldPaintEvent, 
                                           PaintEvent newPaintEvent)
    {
	PaintEvent ret;
	if (newPaintEvent.id == PaintEvent.PAINT) {
            ret = oldPaintEvent;
        } else {
	    ret = newPaintEvent;
        }
        ret.setUpdateRect((oldPaintEvent.getUpdateRect()).union(
                           newPaintEvent.getUpdateRect()));
	return ret;
    }
}
